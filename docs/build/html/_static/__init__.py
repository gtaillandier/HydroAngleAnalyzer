...
class TabsNode(nodes.container):
    pass
...
class TabBarNode(nodes.Part, nodes.Element):
    pass


class TabButtonNode(nodes.Part, nodes.Element):
    pass
...
def visit_tab_latex(self, node):
    if "texttab" in node["classes"]:
        self.body.append(
            r"\sphinxSetupCaptionForVerbatim{{{}}}".format(
...
def add_assets(app):
    ...
    if app.builder.name in _latex_builders:
        copy_asset_file(os.path.join(package_dir, STY_FILE), app.builder.outdir)
...
def setup(app):
    ...
    app.add_node(TabButtonNode, html=(visit_tabbutton_html, depart_tabbutton_html))
    app.add_node(
        TabNode,
