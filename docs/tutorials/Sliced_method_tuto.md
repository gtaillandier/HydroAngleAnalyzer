...
**Example README:** `docs/examples/contact_angle_sliced/README.md`
````markdown
# Contact Angle Analysis (Sliced Method)

This example demonstrates how to use the **sliced contact angle method** in `hydroangleanalyzer`.

The sliced method divides the droplet into horizontal slices and fits a spherical model to determine the contact angle at the liquid–solid interface.

## Steps
1. Load a LAMMPS dump trajectory (`traj_spherical_drop_4k.lammpstrj`).
2. Identify water oxygen atoms.
3. Initialize the parser and analyzer using the `sliced` method.
4. Run the analysis and print results.

## Running the Example

To run:
```bash
python example_sliced.py
```

## Output

- Number of water molecules
- Computed contact angle
- Optional plots and raw data in `result_dump_spherical_sliced/`

## Documentation

For full explanation, see:
 `../../tutorials/sliced_method_tutorial.md`
````