...
class Ase_Parser(BaseParser):
    def __init__(self, in_path: str) -> None:
        ...
        try:
            ...
            raise ImportError(
                "The 'ase' package is required to use Ase_Parser. "
...
class ASE_WaterMoleculeFinder:
    ...
    def __init__(
        ...
        # Store ASE objects for later use
        self._ase_read = read
    ...
    def get_water_oxygen_positions(self, num_frame: int) -> np.ndarray:
        ...
        Args:
            num_frame: Index of the frame in the trajectory.
...
class Ase_wallParser:
    def __init__(self, in_path, particule_liquid_type):
        """
        Initialize the Ase_wallParser.
        Args:
            in_path: Path to the input trajectory file.
            particule_liquid_type: List of particle types to exclude (liquid particles).
        """
        try:
            from ase.io import read
        except ImportError as e:
            raise ImportError(
                "The 'ase' package is required to use Ase_wallParser. "
                "Install it with: pip install ase"
            ) from e
        self.in_path = in_path
