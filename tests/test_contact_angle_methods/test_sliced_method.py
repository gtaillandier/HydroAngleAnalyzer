...
@pytest.fixture
def filename():
    return pathlib.Path(__file__).parent / ".." /"trajectories" / "traj_spherical_drop_4k.lammpstrj"
...
def test_contact_angle_sliced_with_real_data(parser, oxygen_indices):
    ...
    max_dist = int(np.max(np.array([parser.box_size_y(num_frame=0), parser.box_size_x(num_frame=0)])) / 2)
...
def test_sliced_contact_angle_analyzer_with_real_data(filename, oxygen_indices, tmp_path):
